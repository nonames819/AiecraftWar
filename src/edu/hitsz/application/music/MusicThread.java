...
public class MusicThread extends Thread {
    ...
    public MusicThread(String filename, boolean needCircle) {
        //初始化filename
    ...
    public void play(InputStream source) {
        ...
        try {
            ...
            while (numBytesRead != -1 && !needStop) {
                ...
                int offset = 0;
				//通过此源数据行将数据写入混频器
                if (numBytesRead != -1 && !needStop) {
                    dataLine.write(buffer, offset, numBytesRead);
                    offset += byteInterval;
                    numBytesRead -= byteInterval;
                }
            }
